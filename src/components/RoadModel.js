...
export function RoadModel() {
   ...
   * @type {Boolean[]}
   */
  const peopleQueueArr = useMemo(() => new Array(Math.min(20, peopleQueue + peopleMove)).fill(false).map((p, i) => i < peopleMove), [peopleQueue, peopleMove])

  /**
   * @type {Boolean[]}
   */
  const autoQueueArr = useMemo(() => new Array(Math.min(2, autoQueue + autoMove)).fill(false).map((p, i) => i < autoMove), [autoQueue, autoMove])
  ...
  return <svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080" viewBox="0 0 1920 1080" style={{
    ...
    <g clipPath="url(#screen)">
      <g transform="translate(960, 540)">
        ...
        <g onClick={testP}>
          {peopleQueueArr.map((p, i) => <g key={i}
        ...
        <g onClick={testA}>
          {autoQueueArr.map((a, i) => <g key={i} transform={`translate(${290 + (i * 530)}, 0)`}>
